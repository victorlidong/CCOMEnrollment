...
namespace university.BLL
{
    public partial class siteconfig
    {
        ...
        {
            Model.siteconfig model = CacheHelper.Get<Model.siteconfig>(MyKeys.CACHE_SITE_CONFIG);
            if (model == null)
            {
                CacheHelper.Insert(MyKeys.CACHE_SITE_CONFIG, dal.loadConfig(configFilePath), configFilePath);
                model = CacheHelper.Get<Model.siteconfig>(MyKeys.CACHE_SITE_CONFIG);
            }
            return model;
        }
        /// <summary>
        ...
        {
            Model.siteconfig model = CacheHelper.Get<Model.siteconfig>(MyKeys.CACHE_SITE_CONFIG_CLIENT);
            if (model == null)
            {
                model = dal.loadConfig(configFilePath);
                model.templateskin = model.webpath + "templates/" + model.templateskin;
                CacheHelper.Insert(MyKeys.CACHE_SITE_CONFIG_CLIENT, model, configFilePath);
            }
            return model;
        }
