...
{
    public partial class SelectToPreliminary : university.UI.ManagePage
    {
        ...
        protected bool hasPrelimary = true;

        public SelectToPreliminary()
        {
        ...
        private void RptBind(string _strWhere, string _order)
        {
            ...
            BLL.CCOM.View_FirstIn_Score bll = new BLL.CCOM.View_FirstIn_Score();
            ...
            string pageUrl = Utils.CombUrlTxt("SelectToPreliminary.aspx", "fun_id={0}&page={1}&keywords={2}&major_id={3}", DESEncrypt.Encrypt(this.fun_id), "__id__",
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("SelectToPreliminary_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("SelectToPreliminary_page_size", _pagesize.ToString(), 43200);
                }
            }
            string keywords = MyRequest.GetQueryString("keywords");
            Response.Redirect(Utils.CombUrlTxt("SelectToPreliminary.aspx", "fun_id={0}&keywords={1}&major_id={2}", DESEncrypt.Encrypt(this.fun_id),
                this.txtKeywords.Text, DESEncrypt.Encrypt(this.major_id)));
        }
        #endregion
        ...
        public string getIntoPreliminary(string _up_calculation_status)
        {
        ...
        protected void btnSetFirstIn_Click(object sender, EventArgs e)
        {
            Response.Redirect(Utils.CombUrlTxt("SetFirstIn.aspx", "fun_id={0}&major_id={1}", DESEncrypt.Encrypt(this.fun_id),
                DESEncrypt.Encrypt(this.major_id)));
        }
        ...
        {
           string order = "Efss_score DESC, Efss_sequence DESC ,UP_CCOM_number asc";
           string strWhere = CombSqlTxt(this.keywords);
           ...
           try
           {
               ...
               BLL.CCOM.View_FirstIn_Score bll = new BLL.CCOM.View_FirstIn_Score();
               List<Model.CCOM.View_FirstIn_Score> modelList = bll.GetModelList(strWhere + " order by " + order);
               ...
               for (int i = 0, num = 1; i < _count; i++, num++)
               {
                   ...
                   dr["序号"] = num.ToString();
                   dr["状态"] = getIntoPreliminary(modelList[i].UP_calculation_status.ToString());
                   ...
                   dr["总成绩"] = ((decimal)(modelList[i].Efss_score)).ToString("F2");
                   dr["总平均序"] = ((decimal)modelList[i].Efss_sequence).ToString("F2");

                   _ds.Tables["Sheet1"].Rows.Add(dr);
               }
               DataToExcel.ExportToExcel(_ds, Server.MapPath("/upload/excel/"), "初试一轮情况_" + DateTime.Now.ToFileTime().ToString() + ".xlsx", this.Page);
           }
           catch
           {
               JscriptMsg("获取初试一轮情况出错", "", "Error");
           }
        }
    }
}