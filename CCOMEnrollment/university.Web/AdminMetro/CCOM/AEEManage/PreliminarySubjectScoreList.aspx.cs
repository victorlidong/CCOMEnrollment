...
{
    public partial class PreliminarySubjectScoreList : university.UI.ManagePage
    {
        ...
        public PreliminarySubjectScoreList()
        {
        ...
        private void RptBind(string _strWhere, string _order)
        {
            ...
            string pageUrl = Utils.CombUrlTxt("PreliminarySubjectScoreList.aspx", "fun_id={0}&page={1}&keywords={2}&major_id={3}", DESEncrypt.Encrypt(this.fun_id), "__id__",
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("PreliminarySubjectScoreList_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("PreliminarySubjectScoreList_page_size", _pagesize.ToString(), 43200);
                }
            }
            string keywords = MyRequest.GetQueryString("keywords");
            Response.Redirect(Utils.CombUrlTxt("PreliminarySubjectScoreList.aspx", "fun_id={0}&keywords={1}&major_id={2}", DESEncrypt.Encrypt(this.fun_id),
                this.txtKeywords.Text, DESEncrypt.Encrypt(this.major_id)));
        }
        #endregion
        ...
        protected void btnSetPreliminary_Click(object sender, EventArgs e)
        {
            Response.Redirect(Utils.CombUrlTxt("SetPreliminarySubject.aspx", "fun_id={0}&major_id={1}", this.fun_id,
        ...
        protected void exportexcel_ServerClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataToExcel.ExportToExcel(_ds, Server.MapPath("/upload/excel/"), "初试情况_" + DateTime.Now.ToFileTime().ToString() + ".xlsx", this.Page);
            }
            catch
            {
                JscriptMsg("获取初试情况出错", "", "Error");
            }
        }
    }
}