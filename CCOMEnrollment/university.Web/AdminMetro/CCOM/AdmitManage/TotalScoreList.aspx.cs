...
{
    public partial class TotalScoreList : university.UI.ManagePage
    {
        ...
        private void RptBind(string _strWhere, string _order)
        {
            ...
            string pageUrl = Utils.CombUrlTxt("TotalScoreList.aspx", "fun_id={0}&page={1}&keywords={2}&major_id={3}", DESEncrypt.Encrypt(this.fun_id), "__id__",
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("TotalScoreList"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("TotalScoreList_page_size", _pagesize.ToString(), 43200);
                }
            }
            string keywords = MyRequest.GetQueryString("keywords");
            Response.Redirect(Utils.CombUrlTxt("TotalScoreList.aspx", "fun_id={0}&keywords={1}&major_id={2}", DESEncrypt.Encrypt(this.fun_id),
                this.txtKeywords.Text, this.major_id));
        }
        #endregion
        protected void btnGetCEE_Click(object sender, EventArgs e)
        {
            new Calculation().Calculate(major_id);
            Response.Redirect(Utils.CombUrlTxt("TotalScoreList.aspx", "fun_id={0}&keywords={1}&major_id={2}", DESEncrypt.Encrypt(this.fun_id),
                this.txtKeywords.Text, DESEncrypt.Encrypt(this.major_id)));
        }
        ...
        protected void exportexcel_ServerClick(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               */
                ...
                DataToExcel.ExportToExcel(ds, Server.MapPath("/upload/excel/"), this.ddlMajor.SelectedItem.Text + "_最终成绩汇总_" + DateTime.Now.ToFileTime().ToString() + ".xlsx", this.Page);
            }
            catch
            {
