...
namespace university.Common
{
    public class Utils
    {
        ...
        {
            return MyKeys.ASSEMBLY_VERSION;
        }
        #endregion
        ...
        /// <param name="n">数字位数n（大于0）</param>
        /// <returns></returns>
        public static bool IsNBitsNumber(string expression, int n)
        {
            n = n < 0 ? 0 : n;
        ...
        public static bool IsSafeSqlString(string _str)
        {
        ...
        /// <param name="centSize">中间页码数量</param>
        /// <returns></returns>
        public static string OutPageList(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize, bool isMetro)
        {
            //计算页数
            if (totalCount < 1 || pageSize < 1)
            {
            ...
            pageStr.Append("<ul>");
            string pageId = "__id__";
            string firstBtn = "<li class='prev'><a href=\"" + ReplaceStr(linkUrl, pageId, (pageIndex - 1).ToString()) + "\">«上一页</a></li>";
            string lastBtn = "<li class='next'><a href=\"" + ReplaceStr(linkUrl, pageId, (pageIndex + 1).ToString()) + "\">下一页»</a></li>";
            ...
            if (pageIndex <= 1)
            {
                firstBtn = "<li  class='prev disabled'><a href='javascript:void(0);'>«上一页</a></li>";
            }
            if (pageIndex >= pageCount)
            {
                lastBtn = "<li class='next disabled'><a href='javascript:void(0);'>下一页»</a></li>";
            }
            if (pageIndex == 1)
            {
        ...
        public static string OutOperList(String operName, String operUrl, Dictionary<Object, Object> itemOperIdKv)
        {
            if (itemOperIdKv == null || itemOperIdKv.Count < 1)
                return "";
            StringBuilder pageStr = new StringBuilder();
            String operId = "__id__";
            foreach (var item in itemOperIdKv)
            {
                pageStr.Append("<p>").Append(item.Value);
                string oper = "&nbsp;&nbsp;<a style='color:red;' href=\"" + ReplaceStr(operUrl, operId, item.Key.ToString()) + "\">" + operName + "</a>";
                pageStr.Append(oper);
                pageStr.Append("</p>");
            }
            return pageStr.ToString();
        ...
        public static string[] GetHtmlImageUrlList(string sHtmlText)
        {
            ...
            foreach (Match match in matches)
                sUrlList[i++] = match.Groups["imgUrl"].Value.ToLower();
            return sUrlList;
        }
        ...
        #region "UserType转换"
        /// <summary>
        /// web定义的UserType:   0-校园用户(SchoolUser)  1-社会用户(SocialUser)  2-管理员用户(AdminUser)
        /// token解析后定义的UserType:   0-管理员用户   1-校园用户   2-社会用户
        /// </summary>
        /// <param name="userType">待转换的UserType</param>
        /// <param name="isFromToken">若为true，将app token解析的UserType转换为Web的UserType; 若为false，则相反</param>
        /// <returns>返回转换后的UserType; 若不存在对应，则返回-1</returns>
        public static Int32 ConvertUserType(Int32 userType, bool isFromToken)
        {
            var userType_TokenToWeb = new Dictionary<Int32, Int32> { { 0, 2 }, { 1, 0 }, { 2, 1 } };
            var userType_WebToToken = new Dictionary<Int32, Int32> { { 0, 1 }, { 1, 2 }, { 2, 0 } };
            if (isFromToken)
            {
                if (userType_TokenToWeb.ContainsKey(userType))
                    return userType_TokenToWeb[userType];
            }
            else
            {
                if (userType_WebToToken.ContainsKey(userType))
                    return userType_WebToToken[userType];
            }
            return -1;
        }
        #endregion
        ...
        /// 判断文件后缀是否合法
        /// </summary>
        /// <param name="fileExt">文件后缀名，如"rar"、"doc"，前面没有"."</param>
        /// <returns></returns>
        public static bool IsLegalFileExt(String fileExt)
        {
            string[] excExt = { "gif", "tiff","jpg", "jpeg","png","bmp", "rar", "zip", "doc", "xls", "txt", "pdf", "doc", "docx", "ppt", "pptx", "xls", "xlsx" };
        ...
        /// <returns></returns>
        public static bool IsImgExt(String fileExt)
        {
            string[] excExt = { "gif", "tiff", "jpg", "jpeg", "png", "bmp" };
            for (int i = 0; i < excExt.Length; i++)
            {
                if (excExt[i].ToLower() == fileExt.ToLower())
                {
        ...
        /// <summary>
        /// 判断是否为Office文档后缀(仅Microsoft Office)
        /// </summary>
        /// <param name="fileExt"></param>
        /// <returns></returns>
        public static bool IsOfficeExt(String fileExt)
        {
            string[] excExt = {"doc", "docx", "ppt", "pptx", "xls", "xlsx" };
            for (int i = 0; i < excExt.Length; i++)
            {
                if (excExt[i].ToLower() == fileExt.ToLower())
                {
        ...
        /// <param name="fileExt"></param>
        /// <returns></returns>
        public static bool IsTxtExt(String fileExt)
        {
            string[] excExt = { "txt"};
            for (int i = 0; i < excExt.Length; i++)
            {
        ...
        /// <param name="fileExt"></param>
        /// <returns></returns>
        public static bool IsPdfExt(String fileExt)
        {
            string[] excExt = { "pdf" };
            for (int i = 0; i < excExt.Length; i++)
            {
        ...
        /// <summary>
        /// 在相对目录路径下创建日期目录(yyyyMM/dd/)，dt为设定的时间，并返回最终拼接的目录路径
        /// 如：相对目录/upload/news/，返回/upload/news/201505/08/
        /// </summary>
        /// <param name="relativeBaseDir">相对目录，如/upload/news/</param>
        /// <returns></returns>
        public static String GetDateDirPath(String relativeBaseDir,DateTime dt)
        {
            String dateDir = dt.ToString("yyyyMM") + "/" + dt.ToString("dd") + "/";
            var baseDir = relativeBaseDir + dateDir;
        ...
        /// <summary>
        /// 在相对目录路径下创建日期目录(yyyyMM/dd/)，时间默认为当前时间，并返回最终拼接的目录路径
        /// 如：相对目录/upload/news/，返回/upload/news/201505/08/
        /// </summary>
        /// <param name="relativeBaseDir">相对目录，如/upload/news/</param>
        /// <returns></returns>
        public static String GetDateDirPath(String relativeBaseDir)
        {
            return GetDateDirPath(relativeBaseDir, DateTime.Now);
        }
