...
namespace university.UI
{
    ...
    public class HttpModule : System.Web.IHttpModule
    {
        ...
        private void ReUrl_BeginRequest(object sender, EventArgs e)
        {
            ...
            Model.siteconfig siteConfig = new BLL.siteconfig().loadConfig(Utils.GetXmlMapPath(MyKeys.FILE_SITE_XML_CONFING)); //获得站点配置信息
            ...
            switch (siteConfig.staticstatus)
            {
                case 0: //关闭重写
                    ...
                    {
                        context.RewritePath(siteConfig.webpath + MyKeys.DIRECTORY_REWRITE_ASPX + "/" + requestPage);
                    }
                    break;
                case 1: //伪URL重写
                    if (isRewritePath)
                ...
                case 2: //全静态
                    ...
                    {
                        context.RewritePath(siteConfig.webpath + MyKeys.DIRECTORY_REWRITE_HTML + "/index." + siteConfig.staticextension);
                    }
                    break;
            }
        ...
        private void RewriteUrl(HttpContext context, string dirPath, string requestPath, string requestPage)
        {
            ...
            //        context.RewritePath(dirPath + MyKeys.DIRECTORY_REWRITE_ASPX + "/" + url.page, string.Empty, newUrl);
            //        return;
            //    }
            //}
            if (IsAspxFile(requestPath))
            {
                context.RewritePath(dirPath + MyKeys.DIRECTORY_REWRITE_ASPX + "/" + requestPage);
            }
        }
        #endregion
    ...
    public class SiteUrls
    {
        ...
        string SiteUrlsFile = Utils.GetXmlMapPath(MyKeys.FILE_SITE_XML_CONFING);
        ...
        {
            SiteUrls _cache = CacheHelper.Get<SiteUrls>(MyKeys.CACHE_SITE_HTTP_MODULE);
            lock (lockHelper)
            {
                if (_cache == null)
                {
                    CacheHelper.Insert(MyKeys.CACHE_SITE_HTTP_MODULE, new SiteUrls(), Utils.GetXmlMapPath(MyKeys.FILE_URL_XML_CONFING));
                    instance = CacheHelper.Get<SiteUrls>(MyKeys.CACHE_SITE_HTTP_MODULE);
                }
            }
            return instance;
