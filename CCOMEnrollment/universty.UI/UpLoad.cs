    ...
    public class UpLoad
    {
        ...
        public class UploadConfig
        {
            public class LinuxSite
            {
                ...
                public string nginx_root;
            }
            public LinuxSite linux_site;
            public static UploadConfig upConfig = null;
            public UploadConfig()
            {
                string xmlpath = HttpContext.Current.Server.MapPath(System.Configuration.ConfigurationManager.AppSettings["FileServerpath"].ToString());
                XmlDocument xd = new XmlDocument();
                xd.Load(xmlpath);
                XmlNode root = xd.DocumentElement;
                linux_site = new LinuxSite();
                linux_site.host = root.SelectSingleNode("linux_site/host").InnerText;
                linux_site.url = root.SelectSingleNode("linux_site/url").InnerText;
                linux_site.user = root.SelectSingleNode("linux_site/user").InnerText;
                linux_site.port = root.SelectSingleNode("linux_site/port").InnerText;
                linux_site.password = root.SelectSingleNode("linux_site/password").InnerText;
                linux_site.nginx_root = root.SelectSingleNode("linux_site/nginx_root").InnerText;
            }
        ...
        public class ThreadWithState
        {
            ...
            public ThreadWithState(string filename, string path, string windowsroot, string linuxroot)
            {
                this.filename = filename;
                this.path = path;
                this.windowsroot = windowsroot;
                this.linuxroot = linuxroot;
            }
        ...
        public string fileSaveAs(HttpPostedFile postedFile, bool isThumbnail, bool isWater, bool _isImage, bool _isReOriginal)
        {
            try
            {
                ...
                //保存文件到linux文件服务器
                UploadFileThread(dirPath + fileName);
        ...
        {
            string path = siteConfig.webpath + siteConfig.attachpath + "/temp/"; //站点目录+上传目录
